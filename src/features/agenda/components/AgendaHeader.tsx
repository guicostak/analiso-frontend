...
import { ChevronLeft, ChevronRight, SlidersHorizontal, ChevronDown, Check, CalendarDays } from 'lucide-react';
...
export function AgendaHeader({ navigation, filters, uniqueTickers }: AgendaHeaderProps) {
  ...
  const viewLabel = navigation.viewMode === 'semana' ? 'Semana' : 'Mês';
  ...
  return (
    <div className="flex shrink-0 items-center justify-between gap-4 border-b border-border bg-card px-5 py-3">
      ...
      {/* ── Direita: navegação + filtros + view ───────────────────────────── */}
      <div className="flex items-center gap-2">
        ...
        <div className="flex items-center gap-1">
          <button
            ...
            className="flex h-8 w-8 items-center justify-center rounded-md text-muted-foreground transition-colors duration-150 hover:bg-hover hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
          >
            <ChevronLeft size={16} />
          </button>
          ...
          <button
            ...
            className="min-w-[160px] px-2 text-sm font-medium text-foreground transition-colors hover:text-brand focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring rounded"
        ...
        <div ref={viewRef} className="relative">
          ...
          {viewOpen && (
            <div className="absolute right-0 top-full z-50 mt-2 w-36 overflow-hidden rounded-xl border border-border bg-card shadow-lg dark:shadow-none animate-in fade-in-0 slide-in-from-top-2 duration-150">
              {(['semana', 'mes'] as AgendaViewMode[]).map((mode) => {
                const label  = mode === 'semana' ? 'Semana' : 'Mês';
