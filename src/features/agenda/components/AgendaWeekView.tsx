...
export function AgendaWeekView({ weekDates, events, selectedEvent, onSelectEvent }: AgendaWeekViewProps) {
  ...
  return (
    <div className="flex flex-1 flex-col overflow-hidden bg-card">
      ...
      {/* ── Grid + eventos ───────────────────────────────────────────────── */}
      {/* flex-1 min-h-0: cresce para preencher o espaço disponível no flex-col pai */}
      <div className="flex-1 min-h-0 overflow-y-auto">
        {/*
          height: 100% → preenche o viewport do scroll container
          minHeight    → garante mínimo de ROWS linhas visíveis
          Combinados: a coluna sempre tem altura definida, sem espaço em branco no fundo
        */}
        <div
          className="flex"
          ...
          {weekDates.map((date, idx) => {
            ...
            return (
              <div
                ...
                `}
                style={{
                  /* repeating-linear-gradient repete indefinidamente → cobre qualquer altura */
