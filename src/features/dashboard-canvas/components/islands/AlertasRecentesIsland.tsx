 ...
 *
 * Resumo compacto dos alertas recentes (notificações do tipo "alerta").
 */
...
export function AlertasRecentesIsland(_props: IslandProps) {
  ...
  const alerts = items.filter((i) => i.type === "alerta");
  const unread = alerts.filter((i) => !i.read).length;
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="text-[14px] font-semibold text-foreground">
          {isLoading ? "Carregando…" : `${alerts.length} no histórico · ${unread} não lido(s)`}
        </p>
      </div>
    </Link>
  );
