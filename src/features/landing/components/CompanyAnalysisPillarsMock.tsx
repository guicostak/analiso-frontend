...
const pillarCards: PillarCard[] = [
  {
    ...
    accent: "#0E9384",
    badgeClass: "border-[#AEE3D8] bg-[#F1FCF9] text-brand",
  ...
  },
  {
    ...
    accent: "#D97706",
    badgeClass: "border-[#F6DEA9] bg-[#FFFBEB] text-warning-text",
...
function signalClasses(tone: PillarTone) {
  if (tone === "risk") return "border-danger-border bg-danger-surface text-danger-text";
  if (tone === "attention") return "border-warning-border bg-warning-surface text-warning-text";
  return "border-[#99F6E4] bg-brand-surface text-brand";
}
          ...
          <article className="rounded-2xl border border-border bg-card p-5 shadow-sm">
            <div className="flex flex-wrap items-start justify-between gap-3">
              ...
              <div className="flex flex-wrap items-center gap-2 text-[12px]">
                <span className="inline-flex items-center gap-1.5 rounded-full border border-[#99F6E4] bg-brand-surface px-3 py-1 font-semibold text-brand">
                  3 saudáveis
                </span>
                <span className="rounded-full border border-warning-border bg-warning-surface px-3 py-1 font-semibold text-warning-text">
                  2 atenção
                </span>
              </div>
            </div>
            <div className="mt-3 flex flex-wrap gap-2 text-[12px] text-dim">
              ...
              >
                ...
                >
                  <div className="flex flex-1 flex-wrap items-center gap-x-3 gap-y-1">
                    ...
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        <h2 className="text-[16px] font-bold text-foreground">{pillar.name}</h2>
                        <span className={cx("rounded-full border px-2.5 py-0.5 text-[11px] font-semibold", pillar.badgeClass)}>
                          {pillar.statusLabel}
                        </span>
                        <span className="text-[12px] text-muted-foreground">{pillar.deltaLabel}</span>
                      </div>
                      <p className="mt-0.5 text-[13px] text-muted-foreground">{pillar.summary}</p>
                    </div>
                  </div>
                  ...
                  <div className="grid gap-0 lg:grid-cols-12">
                    <div className="space-y-0 border-b border-border p-5 lg:col-span-4 lg:border-b-0 lg:border-r">
                      <div>
                        ...
                        <p className="mt-2 text-[14px] leading-relaxed text-foreground">{pillar.meaning}</p>
                      </div>
                    ...
                    <div className="p-5 lg:col-span-8">
                      <section className="rounded-xl border border-border bg-muted p-4">
                        <div className="grid gap-4 lg:grid-cols-2">
                          <div>
                            <p className="text-[10px] font-semibold uppercase tracking-[0.1em] text-muted-foreground">{pillar.evidenceLabel}</p>
                            <p className="mt-1 text-[12px] text-muted-foreground">Indicador: {pillar.evidenceMetric}</p>
                            <p className="mt-2 text-[32px] font-bold leading-none text-foreground">{pillar.evidenceValue}</p>
                            <p className="mt-1 text-[12px] text-muted-foreground">Data: {pillar.evidenceDate}</p>
                            <p className="mt-2 text-[13px] text-foreground">
                              <span className="font-semibold">Histórico:</span>{" "}
                              <span className="font-medium text-muted-foreground">{pillar.reference}</span>
                            </p>
                          </div>
                          <div>
                                ...
                                {["5a", "10a"].map((windowOption, index) => (
                                  <button
                                    ...
                                    className={cx(
                                      ...
                                      index === 0
                                        ? "border border-[#99F6E4] bg-brand-surface font-semibold text-brand"
                            ...
                            <div className="flex h-[126px] items-end gap-2 rounded-2xl border border-border bg-card px-3 pb-3 pt-4">
                              {pillar.chart.map((point) => (
                                <div key={point.year} className="flex flex-1 flex-col items-center gap-2">
                                  ...
                                  <div className="text-center">
                                    <p className="text-[10px] font-medium text-muted-foreground">{point.year}</p>
                                    <p className="text-[10px] font-semibold text-foreground">{point.value}</p>
                                  </div>
                                </div>
                              ))}
                            </div>
                      ...
                      <section className={cx("mt-3 rounded-xl border p-4", signalClasses(pillar.signalTone))}>
                        <div className="flex items-start justify-between gap-3">
                          <div className="flex-1">
                            ...
                            <p className="mt-2 text-[14px] font-semibold text-foreground">{pillar.signalTitle}</p>
                            <p className="mt-1 text-[13px] text-dim">{pillar.signalBody}</p>
                          </div>
                          <button type="button" className="flex-shrink-0 text-[12px] text-brand hover:underline">
                            Ver fonte
                          </button>
                        </div>
                      ...
                      <section className="mt-3 rounded-xl border border-[#99F6E4] bg-gradient-to-br from-[#F0FDFA] to-[#F6FAFC] p-4">
                        <p className="text-[13px] text-dim">Feche a leitura deste pilar com uma ação útil e verificável.</p>
