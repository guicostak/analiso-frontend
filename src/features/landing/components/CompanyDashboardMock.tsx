      ...
      <div className="border-b border-border bg-card px-6 py-4">
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="flex flex-col items-end gap-3">
            <span className="text-[12px] font-medium text-muted-foreground">Referência 23 mar 2026</span>
            <button
              type="button"
      ...
      <div className="flex-1 overflow-y-auto p-5 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        ...
        <div className="col-span-7 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <article className="relative min-h-[148px] overflow-hidden rounded-[20px] border border-[#F0CCD7] bg-card px-5 py-5">
              ...
              <div className="mt-9 flex items-start justify-between gap-3">
                <div>
                  <p className="text-[18px] font-semibold text-foreground">WEGE3</p>
            ...
            <article className="relative min-h-[148px] overflow-hidden rounded-[20px] border border-[#CFE9E2] bg-card px-5 py-5">
              ...
              <div className="mt-9 flex items-start justify-between gap-3">
                <div>
                  <p className="text-[18px] font-semibold text-foreground">VALE3</p>
              ...
              {watchlistItems.map((item) => {
                ...
                return (
                  <button
                    key={item.ticker}
                    type="button"
                    className="flex w-full items-center gap-3 rounded-[18px] border border-border bg-card px-4 py-3 text-left transition hover:bg-card"
                  >
                    <img src={item.logo} alt={item.ticker} className="h-10 w-10 rounded-xl object-cover" />
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2">
                        <p className="text-[13px] font-semibold text-foreground">{item.ticker}</p>
                        <span className={`rounded-full border px-2 py-0.5 text-[10px] font-semibold ${tone.chip}`}>
                          {tone.label}
                        </span>
                      </div>
                      <p className="mt-1 text-[13px] font-medium text-dim">{item.title}</p>
        ...
        <div className="col-span-5 space-y-4">
              ...
              {feedItems.map((item) => {
                ...
                return (
                  <div
                    ...
                    className="rounded-[18px] border border-border bg-card px-4 py-3"
                  >
                    ...
                    <p className="mt-2 text-[13px] font-semibold text-dim">{item.title}</p>
          ...
          <article className="rounded-[20px] border border-border bg-card p-5">
              ...
              {pillarMovements.map((item) => (
                <div key={item.pillar}>
                  <div className="mb-1.5 flex items-center justify-between text-[12px]">
                    <span className="font-medium text-dim">{item.pillar}</span>
                    <span className="text-muted-foreground">{item.events} sinais</span>
                  </div>
            ...
            <p className="mt-4 rounded-[16px] border border-border bg-muted px-3 py-3 text-[12px] leading-5 text-[#526070]">
          ...
          <article className="rounded-[20px] border border-border bg-card p-5">
            ...
            <div className="mt-4 space-y-3">
              <button
                type="button"
                className="flex w-full items-center justify-between rounded-[16px] border border-[#D7F3ED] bg-brand-surface px-4 py-3 text-left"
              >
                <div>
                  <p className="text-[13px] font-semibold text-foreground">Abrir WEGE3 primeiro</p>
              ...
              <button
                type="button"
                className="flex w-full items-center justify-between rounded-[16px] border border-border bg-card px-4 py-3 text-left"
              >
                <div>
                  <p className="text-[13px] font-semibold text-foreground">Comparar com LREN3</p>
