    ...
    <div className="fixed inset-0 z-50">
      ...
      <aside className="absolute inset-y-0 right-0 w-full max-w-[460px] overflow-y-auto border-l border-border bg-card p-6 shadow-2xl">
        <div className="mb-6 flex items-start justify-between">
          <div>
            <p className="text-xs uppercase tracking-[0.08em] text-muted-foreground">Evidence drawer</p>
            <h3 className="mt-1 text-lg font-semibold text-foreground">{data.metricName}</h3>
          </div>
          <button onClick={onClose} className="rounded-full border border-border p-1.5 text-muted-foreground">
            <X className="h-4 w-4" />
          </button>
        </div>
        <div className="space-y-4 text-sm">
          ...
          <div>
            <p className="text-[12px] font-semibold text-muted-foreground">Data de atualizacao</p>
            <p className="mt-1 text-foreground">{data.source.updatedAt}</p>
          </div>
          {data.source.reference ? (
            <div>
              <p className="text-[12px] font-semibold text-muted-foreground">Trecho/identificador</p>
...
export function ComparePage() {
  ...
  const qualityTone = useMemo(() => {
    ...
    if (latest && Date.now() - parseDate(latest) < 60 * 24 * 60 * 60 * 1000) return { dot: "bg-success-text", label: "Saudavel" };
        ...
        <div className="px-8 py-8">
            ...
            <div className="grid grid-cols-1 gap-2 xl:grid-cols-12">
                ...
                <div className="mt-1.5 flex flex-wrap items-center gap-1.5">
                  {selected.map((c, i) => (
                    ...
                    >
                      ...
                      <button onClick={() => setSelectedTickers((p) => p.filter((t) => t !== c.ticker))} className="rounded-full p-0.5 hover:bg-hover">
                ...
                <div className="relative mt-2.5 w-full max-w-[420px]">
                  ...
                  {openPicker && selected.length < 4 ? (
                    <div className="absolute z-40 mt-2 w-full rounded-xl border border-border bg-card p-1 shadow-xl">
                      {available.length ? (
                        available.map((c) => (
                          <button key={c.ticker} onClick={() => addTicker(c.ticker)} className="flex w-full items-center justify-between rounded-lg px-3 py-2 text-left hover:bg-hover">
                            <div>
              ...
              <article className="rounded-2xl border border-border bg-card p-2.5 xl:col-span-5">
                ...
                <div className="mt-1.5 flex flex-wrap items-center gap-1.5">
                  ...
                  {canCompare ? <span className="rounded-lg border border-border bg-muted px-2 py-1 text-[11px] text-muted-foreground">Pilar em foco: {PILLAR_LABEL[activePillar]}</span> : null}
                </div>
          ...
          {!canCompare ? (
            <section className="rounded-2xl border border-border bg-card p-8 text-center">
              ...
              <p className="mt-2 text-sm text-muted-foreground">Adicione Empresa A e Empresa B para ver quem esta melhor hoje, onde esta o risco e como confirmar.</p>
              <div className="mt-5 flex flex-wrap justify-center gap-2">
                {["WEGE3", "VALE3", "ITUB4"].map((t) => (
                  <button key={t} onClick={() => addTicker(t)} className="rounded-xl border border-border bg-muted px-3 py-1.5 text-xs font-medium">{t}</button>
                ))}
              </div>
            </section>
          ) : refreshing ? (
            ...
            <div className="space-y-8">
              {verdict && a && b ? (
                <section ref={verdictRef} className="scroll-mt-[160px] grid grid-cols-1 gap-4 xl:grid-cols-12">
                  <article className="rounded-3xl border border-border bg-card p-6 shadow-[0_18px_45px_rgba(15,23,42,0.06)] xl:col-span-8">
                    <p className="text-xs font-semibold uppercase tracking-[0.12em] text-brand">Veredito da comparacao</p>
                    ...
                    <p className="mt-2 text-[15px] font-medium text-foreground">{verdict.consequence}</p>
                    ...
                    <p className="mt-3 text-sm text-muted-foreground">Baseado em 5 pilares | Dados oficiais de CVM, B3 e RI | Atualizado em {verdict.latestUpdate}</p>
                    <div className="mt-4 rounded-2xl border border-border bg-muted p-3">
                      ...
                      <div className="mt-2 grid grid-cols-1 gap-2 sm:grid-cols-3">
                        <div className="rounded-xl bg-card px-3 py-2">
                          ...
                          <p className="text-base font-semibold text-brand">{n(scoreboard?.avgA ?? 0, 1)}</p>
                        </div>
                        <div className="rounded-xl bg-card px-3 py-2">
                          <p className="text-[11px] text-muted-foreground">{b.ticker}</p>
                          <p className="text-base font-semibold text-dim">{n(scoreboard?.avgB ?? 0, 1)}</p>
                        </div>
                        <div className="rounded-xl bg-card px-3 py-2">
                          <p className="text-[11px] text-muted-foreground">Diferenca</p>
                          <p className="text-base font-semibold text-foreground">{n(Math.abs((scoreboard?.avgA ?? 0) - (scoreboard?.avgB ?? 0)), 1)} pts</p>
                        </div>
                      </div>
                      <p className="mt-2 text-[12px] text-muted-foreground">Use o score para calibrar intensidade, nao para substituir o contexto dos pilares.</p>
                    </div>
                  ...
                  <div className="space-y-4 xl:col-span-4">
                    <article className="rounded-2xl border border-border bg-card p-5">
                      <p className="inline-flex items-center gap-1.5 text-xs font-semibold uppercase tracking-[0.08em] text-foreground"><TriangleAlert className="h-3.5 w-3.5 text-warning-text" />Onde olhar com mais cuidado</p>
                      <p className="mt-2 text-sm text-foreground">
                        {verdict.keyRisk?.loser.ticker} exige mais atencao em {PILLAR_LABEL[verdict.keyRisk?.p ?? "Divida"]}.
                      </p>
                      <p className="mt-1 text-xs text-muted-foreground">Pior score identificado: {n(verdict.keyRisk?.lowestScore ?? 0, 1)}/10</p>
                    </article>
                    <article className="rounded-2xl border border-border bg-card p-5">
                      <p className="inline-flex items-center gap-1.5 text-xs font-semibold uppercase tracking-[0.08em] text-foreground"><ArrowRightLeft className="h-3.5 w-3.5 text-dim" />Onde a diferenca realmente aparece</p>
                      <p className="mt-2 text-sm text-foreground">
                        {PILLAR_LABEL[verdict.biggestGap?.p ?? "Divida"]} e o pilar que mais separa as empresas.
                      </p>
                      <p className="mt-1 text-xs text-muted-foreground">Delta de score: {n(verdict.biggestGap?.delta ?? 0, 1)}/10</p>
                    </article>
                  </div>
                </section>
              ) : null}
              ...
              <section className="rounded-2xl border border-border bg-card p-6">
                ...
                <p className="mt-1 text-sm text-muted-foreground">Comece por aqui para decidir onde focar primeiro.</p>
                <div className="mt-5 grid grid-cols-1 gap-4 lg:grid-cols-3">
                  {topPillarDiffs.map((item) => (
                    ...
                    >
                      <p className="text-sm font-semibold">{PILLAR_LABEL[item.p]}</p>{item.p === activePillar ? <p className="mt-1 text-[11px] font-medium text-brand">Pilar em foco</p> : null}
                      <p className="mt-2 text-[13px] font-medium text-foreground">{item.winner.ticker} leva vantagem.</p><p className="mt-1 text-xs text-dim">{pillarInsight(item.p, item.winner.ticker)}</p>
              ...
              <section className="rounded-2xl border border-border bg-card p-6">
                ...
                <p className="mt-1 text-sm text-muted-foreground">Veja quem vence, quanto separa e por que isso importa na tese.</p>
                <div className="mt-4 grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-5">
                  {otherPillarDiffs.map((item) => (
                    ...
                    >
                      <p className={`text-sm ${item.p === activePillar ? "font-bold text-brand-text" : "font-semibold text-foreground"}`}>{PILLAR_LABEL[item.p]}</p>{item.p === activePillar ? <p className="mt-1 text-[11px] font-semibold text-brand">Pilar em foco</p> : null}
                      <p className="mt-1 text-xs text-muted-foreground">Vence: {item.winner.ticker}</p>
                      <p className="mt-2 text-xs font-semibold text-foreground">Delta {n(item.delta, 1)}/10</p>
                      <p className="mt-2 text-[11px] text-dim">{pillarConsequence(item.p, item.delta, item.winner.ticker)}</p>
                      <p className="mt-2 text-[11px] font-semibold text-brand-text transition-colors group-hover:text-brand">Ver detalhe</p>
                    </button>
                  ))}
                </div>
              </section>

              <section ref={detailRef} className="scroll-mt-[160px] rounded-2xl border border-border bg-card p-6">
                ...
                <div className="mt-4 grid grid-cols-1 gap-4 xl:grid-cols-12">
                  <article className="rounded-2xl border border-border bg-muted p-5 xl:col-span-4">
                    <div className="space-y-4 text-sm">
                      {a && b ? (
                        <div>
                          <p className="text-[12px] font-semibold text-muted-foreground">Resumo do pilar</p>
                          <p className="mt-1.5 text-foreground">
                            {a.pillars[activePillar].score >= b.pillars[activePillar].score ? a.ticker : b.ticker} esta melhor neste pilar hoje.
                          </p>
                        </div>
                      ) : null}
                      <div><p className="text-[12px] font-semibold text-muted-foreground">O que isso quer dizer</p><p className="mt-1.5 text-foreground">{pillarCopy[activePillar].what}</p></div>
                      <div><p className="text-[12px] font-semibold text-muted-foreground">Por que isso pesa na analise</p><p className="mt-1.5 text-foreground">{pillarCopy[activePillar].why}</p></div>
                      <div><p className="text-[12px] font-semibold text-muted-foreground">O que observar com atencao</p><p className="mt-1.5 text-foreground">{pillarCopy[activePillar].how}</p></div>
                      <div className="flex flex-wrap gap-2">{pillarCopy[activePillar].ranges.map((r) => <span key={r} className="rounded-xl border border-border bg-card px-2 py-1 text-[11px]">{r}</span>)}</div>
                    </div>
                  </article>
                      ...
                      <>
                        <div className="mb-3 rounded-xl border border-border bg-muted px-3 py-2">
                          ...
                          <div className="mt-2 flex flex-wrap gap-2 text-[11px]">
                            <span className="rounded-lg border border-border bg-card px-2 py-1 text-dim">Regra: {a.pillars[activePillar].thresholdLabel}</span>
                            <span className="rounded-lg border border-border bg-card px-2 py-1 text-dim">Direcao desejavel: {a.pillars[activePillar].thresholdLabel.toLowerCase().includes("menor") ? "linha descendente" : "linha ascendente"}</span>
                            {latestChartLeader ? <span className="rounded-lg border border-brand-border bg-brand-surface px-2 py-1 font-medium text-brand-text">Vantagem atual: {latestChartLeader}</span> : null}
                            {latestChartDelta !== null ? <span className="rounded-lg border border-border bg-card px-2 py-1 text-dim">Delta final: {n(latestChartDelta, 1)}</span> : null}
                          </div>
                          {scoreVsChartContext ? (
                            <p className="mt-2 rounded-lg border border-warning-border bg-warning-surface px-2.5 py-2 text-[12px] text-warning-text">{scoreVsChartContext}</p>
                          ) : null}
                        </div>
                        <div className="mb-2 flex items-center gap-4 text-xs text-muted-foreground">
                          <span className="inline-flex items-center gap-1"><span className="h-2.5 w-2.5 rounded-full bg-brand" />{a.ticker}</span>
                          <span className="inline-flex items-center gap-1"><span className="h-2.5 w-2.5 rounded-full bg-dim" />{b.ticker}</span>
                        </div>
                        <div className="h-[280px] rounded-xl border border-border bg-card p-3">
                          <ResponsiveContainer width="100%" height="100%">
                            <LineChart data={chartData}>
                              ...
                              <Tooltip content={({ active, payload, label }) => !active || !payload?.length ? null : <div className="rounded-lg border border-border bg-card p-2 text-xs shadow-lg"><p className="font-semibold">Ano: {label}</p>{payload.map((item) => <p key={`${item.name}-${label}`} style={{ color: item.color as string }}>{item.name}: {n(Number(item.value), 2)}</p>)}</div>} />
              ...
              <section className="scroll-mt-[160px] rounded-2xl border border-border bg-card p-6">
                ...
                <p className="mt-1 text-sm text-muted-foreground">Compare numeros completos, tendencia e winner por item.</p>
                {activePillarWinnerSummary ? <p className="mt-2 text-sm font-medium text-dim">{activePillarWinnerSummary}</p> : null}
                {a && b ? (
                    ...
                    <table className="min-w-full divide-y divide-[#E7EAEE] text-sm">
                      ...
                      <tbody className="divide-y divide-[#E7EAEE]">
                        {tableRows.map((row) => {
                          ...
                          return (
                            <tr key={row.name}>
                              <td className="px-3 py-3 align-top"><p className="font-medium">{row.name}</p><p className="mt-1 text-[11px] text-muted-foreground">{row.definition}</p></td>
                              <td className="px-3 py-3 align-top"><p className={`text-brand ${w === "a" ? "font-semibold" : "font-medium"}`}>{formatMetric(row.a?.value ?? null, row.unit)}</p>{row.a ? <p className="mt-1 inline-flex items-center gap-1 text-[11px] font-medium text-muted-foreground">{trendIcon(row.a.trend)}{trendLabel[row.a.trend]}</p> : <p className="mt-1 text-[11px] text-danger-text">Dados indisponiveis</p>}</td>
                              <td className="px-3 py-3 align-top"><p className={`text-dim ${w === "b" ? "font-semibold" : "font-medium"}`}>{formatMetric(row.b?.value ?? null, row.unit)}</p>{row.b ? <p className="mt-1 inline-flex items-center gap-1 text-[11px] font-medium text-muted-foreground">{trendIcon(row.b.trend)}{trendLabel[row.b.trend]}</p> : <p className="mt-1 text-[11px] text-danger-text">Dados indisponiveis</p>}</td>
                              <td className="px-3 py-3 align-top"><span className="inline-flex rounded-md border border-border bg-muted px-2 py-1 text-xs font-semibold text-foreground">{d === null ? "Dados indisponiveis" : `${n(d, row.unit === "x" ? 2 : 1)} ${row.unit}`}</span></td>
                              <td className="px-3 py-3 align-top">{w === "a" ? <span className="inline-flex items-center gap-1 rounded-lg border border-brand-border bg-brand-surface px-2 py-1 text-xs font-semibold text-brand"><Crown className="h-3 w-3" />{a.ticker}</span> : w === "b" ? <span className="inline-flex items-center gap-1 rounded-lg border border-border bg-muted px-2 py-1 text-xs font-semibold text-dim"><Check className="h-3 w-3" />{b.ticker}</span> : <span className="inline-flex rounded-md border border-border bg-muted px-2 py-1 text-[12px] font-semibold text-muted-foreground">Empate</span>}</td>
                              <td className="px-3 py-3 align-top"><p className={`inline-flex rounded-md px-2 py-1 text-xs font-semibold ${d !== null && d >= 1.2 ? "bg-brand-surface text-brand-text" : "bg-muted text-dim"}`}>{evidenceReadLabel(d)}</p><p className="mt-1 text-[11px] text-muted-foreground">{w === "tie" ? "Sem separacao decisiva." : `${w === "a" ? a.ticker : b.ticker} com leitura mais favoravel neste item.`}</p></td>
                              <td className="px-3 py-3 align-top"><button title="Ver fonte" onClick={() => openEvidence(row)} className="inline-flex items-center gap-1 rounded-lg border border-border px-2 py-1.5 text-[11px] text-muted-foreground hover:bg-hover"><FileText className="h-3.5 w-3.5" />Fonte</button></td>
                            </tr>
                          );
                        })}
                      </tbody>
              ...
              <section className="rounded-2xl border border-border bg-card p-6">
                ...
                <p className="mt-1 text-sm text-muted-foreground">Contexto recente + como verificamos os dados.</p>
                <div className="mt-3 rounded-xl border border-brand-border bg-brand-surface px-3.5 py-3 shadow-[0_6px_14px_rgba(14,147,132,0.08)]">
                  <p className="text-[11px] font-semibold uppercase tracking-[0.08em] text-muted-foreground">Sintese principal (90 dias)</p>
                ...
                <div className="mt-4 space-y-3">
                  <button onClick={() => setEventsOpen((v) => !v)} className="flex w-full items-center justify-between rounded-2xl border border-border bg-muted px-4 py-3 text-left"><span className="text-sm font-semibold">Eventos recentes (90 dias) <span className="font-normal text-muted-foreground">- {recentEvents.length} eventos - {eventsOnActivePillar} impactam {PILLAR_LABEL[activePillar]}</span></span>{eventsOpen ? <ChevronUp className="h-4 w-4 text-muted-foreground" /> : <ChevronDown className="h-4 w-4 text-muted-foreground" />}</button>
                  {eventsOpen ? <div className="space-y-2">{recentEvents.length ? recentEvents.map((e) => <article key={e.id} className="rounded-2xl border border-border bg-card p-4"><div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground"><span>{e.date}</span><span className={`rounded-xl px-2 py-0.5 ${e.type === "Fato relevante" ? "bg-warning-surface text-warning-text" : "border border-border"}`}>{e.type}</span><span className="rounded-xl border border-border px-2 py-0.5">{PILLAR_LABEL[e.impact]}</span><span className="rounded-xl border border-border px-2 py-0.5">{e.ticker}</span></div><p className="mt-2 text-sm">{e.summary}</p><button onClick={() => setEvidence({ metricName: `Evento: ${e.type}`, definition: e.summary, unit: "", source: e.source, aTicker: a?.ticker ?? "A", bTicker: b?.ticker ?? "B", aValue: null, bValue: null })} className="mt-3 inline-flex items-center gap-2 rounded-xl border border-border px-2.5 py-1.5 text-xs text-muted-foreground"><FileText className="h-3.5 w-3.5" />Ver fonte</button></article>) : <div className="rounded-2xl border border-border bg-card p-4 text-sm text-muted-foreground">Nenhum evento relevante encontrado para as empresas selecionadas.</div>}</div> : null}
                  <button onClick={() => setQualityOpen((v) => !v)} className="flex w-full items-center justify-between rounded-2xl border border-border bg-muted px-4 py-3 text-left"><span className="text-sm font-semibold inline-flex items-center gap-2"><span className={`h-2.5 w-2.5 rounded-full ${qualityTone.dot}`} />Como verificamos os dados <span className="font-normal text-muted-foreground">- Atualizado em {pair.map((c) => c.updatedAt).sort((x, y) => parseDate(y) - parseDate(x))[0] ?? "-"} - Fontes: CVM/B3/RI</span></span>{qualityOpen ? <ChevronUp className="h-4 w-4 text-muted-foreground" /> : <ChevronDown className="h-4 w-4 text-muted-foreground" />}</button>
                  {qualityOpen ? <div className="grid grid-cols-1 gap-3 lg:grid-cols-2">{pair.map((c, i) => <article key={c.ticker} className="rounded-2xl border border-border bg-card p-4 text-sm"><p className="font-semibold">{i === 0 ? "Empresa A" : "Empresa B"}: {c.ticker}</p><p className="mt-1 text-muted-foreground">Fonte primaria: {c.primarySource}</p><p className="mt-1 text-muted-foreground">Ultima atualizacao: {c.updatedAt}</p><p className="mt-2 text-xs text-muted-foreground" title="Confiabilidade baseada em cobertura, recorrencia e completude">Confianca: {c.confidence}</p>{c.gaps.length ? <div className="mt-2 rounded-xl border border-warning-border bg-warning-surface px-3 py-2 text-xs text-warning-text"><p className="inline-flex items-center gap-1 font-medium"><TriangleAlert className="h-3.5 w-3.5" />Gap identificado</p><p className="mt-1.5 text-foreground">{c.gaps.join(" ")}</p></div> : <div className="mt-2 rounded-xl border border-success-border bg-success-surface px-3 py-2 text-xs text-success-text">Nenhum gap relevante reportado.</div>}</article>)}</div> : null}
                </div>
              </section>
            </div>
          )}

          <p className="mt-8 text-xs text-muted-foreground">Conteudo educacional. Nao constitui recomendacao de compra ou venda.</p>
        </div>
      </main>
      {toast ? <div className="fixed bottom-5 right-5 z-50 rounded-xl border border-brand/20 bg-brand-surface px-3 py-2 text-xs font-medium text-brand-text shadow-lg">{toast}</div> : null}
